...
IOBW_affective = 0.1
fwd_map = [81, 82, 105, 85, 87, 88, 89, 90, 93, 136, 97, 99, 101, 103, 91, 92, 130, \
...
#    print len(index)
    ...
    if options.ost_info == True:
        ...
        for file1 in file_open_set:
            file_list = file_list + '/home/export/online1/swstorage' + file1
            file_list += ','
        ost_list = search_ost(file_list)
        if(len(ost_list) == 0):
            file_list.replace('online1','online2');
            ost_list = search_ost(file_list)
            if(len(ost_list) == 0):
        ...
        for fwd in fwd_file_map:
            ...
            if(len(ost_list) == 0):
                ...
                if(len(ost_list) == 0):
                    ...
                    host = 89
                    bandr, bandw = get_fwd_data(time1, time2, fwd_map[fwd], ost_list, index, host)
                    print "fwd: %s info: \n"%fwd_map[fwd]
                    print bandr
                    print bandw
            else:
