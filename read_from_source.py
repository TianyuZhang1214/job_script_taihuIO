    ...
    for line in reader:
        if(len(line) >= 16):
            ...
            int((line[7].split('-'))[1]) >= 4):
                ...
                if(run_time_tmp > 5*86400 or compute_node_count_tmp > 15000):
...
#    reader = csv.reader(open(other_file_name, 'rU'), dialect=csv.excel_tab)
#    
#    for line in reader:
#        if(len(line) >= 16):
#            if(int((line[7].split('-'))[0]) >= 2017 and \
...
def read_wait_time():
    ...
    for line in reader:
        if(len(line) >= 16):
            ...
            submit_time_format = datetime.datetime.strptime(submit_time, '%Y-%m-%d %H:%M:%S')
            start_time_format = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S')
