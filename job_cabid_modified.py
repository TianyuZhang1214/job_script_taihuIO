...
def save_results(result_file_name, data):
    ...
    writer.writerow(['Program_name', 'JobID', 'Runtime', 'Cabid','Gio'])
...
def get_re(time1, time2):
    ...
    try:
        ...
        sql1 = 'select PROGRAM_NAME,NODELIST,RUNTIME,JOBID,STARTTIME,ENDTIME from JOB_log_all where ' + ite + ' and ' + ite1 + ' and ' + '( ' + item1 + ' or ' + item2 + ' or ' + item3 + ' ) order by PROGRAM_NAME'
        cursor.execute(sql1) 
